        ...
        if (index == i) { // edit product
            for (let key in products[i]) {
                if (key === 'category') {
                    ...
                    tr += `<td><input id="edit-${key}" type="text" value="${products[i][key]}"/></td>`
                }
            }
        } else {
            // do iteration inside an object
...
function showHistory () {
    let table = document.getElementsByTagName('table')[2].tBodies[0]
    
    // do iteration
    let result = []
    for (let i = 0; i < histories.length; i++) {
        let tr = `<tr><td>${i + 1}</td>`
        for (let key in histories[i]) {
            tr += `<td>${histories[i][key]}</td>`
        }
        tr+='</tr>'
        result.push(tr)
    }
